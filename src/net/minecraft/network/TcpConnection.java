...
public class TcpConnection implements INetworkManager {
   ...
   public void addToSendQueue(Packet par1Packet) {
      if (!par1Packet.hasBeenAddedToTcpSendQueue()) {
         par1Packet.compressPayload();
         par1Packet.setHasBeenAddedToTcpSendQueue();
      }

      if (!Main.isPacketIgnored(this.theNetHandler, par1Packet)) {
   ...
   public void processReadPackets() {
      ...
      while(true) {
         ...
         do {
            ...
            var2 = (Packet)this.readPackets.poll();
         } while(is_MITE_DS_client_player && !Main.isPacketThatMITEDSClientPlayerCanSendOrReceive(var2));
   ...
   public int clearReceivedPackets() {
      int num_packets;
      for(num_packets = 0; this.readPackets.poll() != null && !this.theNetHandler.isConnectionClosed(); ++num_packets) {
      }
