...
public class MerchantRecipeList extends ArrayList {
   ...
   public MerchantRecipe canRecipeBeUsed(ItemStack par1ItemStack, ItemStack par2ItemStack, int par3) {
      if (par3 > 0 && par3 < this.size()) {
         ...
         return par1ItemStack.itemID != var6.getItemToBuy().itemID || (par2ItemStack != null || var6.hasSecondItemToBuy()) && (!var6.hasSecondItemToBuy() || par2ItemStack == null || var6.getSecondItemToBuy().itemID != par2ItemStack.itemID) || par1ItemStack.stackSize < var6.getItemToBuy().stackSize || var6.hasSecondItemToBuy() && par2ItemStack.stackSize < var6.getSecondItemToBuy().stackSize ? null : var6;
      } else {
   ...
   public int getPacketSizeOfMerchantRecipeList() {
      ...
      for(int i = 0; i < this.size(); ++i) {
         ...
         bytes += Packet.getPacketSizeOfItemStack(recipe.getItemToBuy());
         bytes += Packet.getPacketSizeOfItemStack(recipe.getItemToSell());
         ++bytes;
         if (recipe.getSecondItemToBuy() != null) {
            bytes += Packet.getPacketSizeOfItemStack(recipe.getSecondItemToBuy());
         }
