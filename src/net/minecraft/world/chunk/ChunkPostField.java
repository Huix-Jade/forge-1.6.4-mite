...
public class ChunkPostField {
   ...
   public ChunkPostField(int type, long hashed_world_seed, int post_max_radius_of_effect, float chance_of_post_per_chunk) {
      this.random = new Random((long)type);
      this.type = type;
      this.random.nextInt();
      ...
      if (this.doesPostFieldRequireRegeneration(chunk_x, chunk_z)) {
         ...
         for(int chunk_dx = -this.field_chunk_range; chunk_dx <= this.field_chunk_range; ++chunk_dx) {
            for(int chunk_dz = -this.field_chunk_range; chunk_dz <= this.field_chunk_range; ++chunk_dz) {
               ...
               long seed = (long)Chunk.getChunkCoordsHash(chunk_x, chunk_z) * this.hashed_type * this.hashed_world_seed;
               this.random.setSeed(seed);
               this.random.nextInt();
               if (this.random.nextFloat() < this.chance_of_post_per_chunk) {
                  this.post_list.add(new ChunkPost(chunk_x, chunk_z, this.random.nextInt(16) * 1, this.random.nextInt(16) * 1, seed));
               }
            }
         }
   ...
   public List getNearbyPostsForBlockCoords(int x, int z) {
      return this.getNearbyPosts(Chunk.getChunkCoordFromBlockCoord(x), Chunk.getChunkCoordFromBlockCoord(z));
   }

   public ChunkPost getNearestPostTo(double pos_x, double pos_z) {
      List posts = this.getNearbyPosts(Chunk.getChunkCoordFromBlockCoord(MathHelper.floor_double(pos_x)), Chunk.getChunkCoordFromBlockCoord(MathHelper.floor_double(pos_z)));
