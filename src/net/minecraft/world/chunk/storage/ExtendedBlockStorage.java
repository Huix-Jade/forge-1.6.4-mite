...
public final class ExtendedBlockStorage
{
	...
	public void setExtBlockID(int par1, int par2, int par3, int par4)
	{
		...
		if (this.blockMSBArray != null)
		{
			...
			Debug.setErrorMessage("setExtBlockID: blockMSBArray was not null");
		}
		...
		if (par4 > 255)
		{
			Debug.setErrorMessage("setExtBlockID: condition was true");
		...
		else if (this.blockMSBArray != null)
		{
			...
			Debug.setErrorMessage("setExtBlockID: blockMSBArray was not null (2)");
		}
	}
	...
	{
		this.skylightArray.set(xz_index, y, brightness);
	}

	public void setExtSkylightValue(int index, int y, int brightness, Chunk chunk)
	{
		this.skylightArray.set(index, y & 15, brightness);

		if (brightness == 0)
		{
			if (chunk.skylight_bottom[index] == y)
			{
				chunk.skylight_bottom[index] = chunk.recalculateSkylightBottom(index, y + 1);
			}
		}
		else if (y < chunk.skylight_bottom[index])
		{
			chunk.skylight_bottom[index] = y;
		}
	}

	public void fillSkylightValues(int brightness)
	{
		Arrays.fill(this.skylightArray.data, (byte)(brightness | brightness << 4));
	}
	...
	{
		return this.skylightArray.get(xz_index, y);
	}
	...
	{
		this.blocklightArray.set(xz_index, y, value);
	}
	...
	{
		return this.blocklightArray.get(xz_index, y);
	}
