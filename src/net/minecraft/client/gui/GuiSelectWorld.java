...
public class GuiSelectWorld extends GuiScreen {
   ...
   public void initButtons() {
      this.buttonList.add(this.buttonSelect = new GuiButton(1, this.width / 2 - 154, this.height - 52, 152, 20, I18n.getString("selectWorld.select")));
      this.buttonList.add(this.button_create = new GuiButton(3, this.width / 2 + 2, this.height - 52, 152, 20, I18n.getString("selectWorld.create")));
      this.buttonList.add(this.buttonRename = new GuiButton(6, this.width / 2 - 154, this.height - 28, 74, 20, I18n.getString("selectWorld.rename")));
      this.buttonList.add(this.buttonDelete = new GuiButton(2, this.width / 2 - 76, this.height - 28, 74, 20, I18n.getString("selectWorld.delete")));
      this.buttonList.add(this.buttonRecreate = new GuiButton(7, this.width / 2 + 2, this.height - 28, 74, 20, I18n.getString("selectWorld.recreate")));
      this.buttonList.add(new GuiButton(0, this.width / 2 + 80, this.height - 28, 74, 20, I18n.getString("gui.cancel")));
   ...
   protected void actionPerformed(GuiButton par1GuiButton) {
      if (par1GuiButton.enabled) {
         if (par1GuiButton.id == 2) {
            ...
            if (var2 != null) {
               this.deleting = true;
               GuiYesNoMITE var3 = getDeleteWorldScreen(this, var2, this.selectedWorld);
   ...
   public boolean selectedWorldPassedValidation() {
      return ((SaveFormatComparator)this.saveList.get(this.selectedWorld)).passed_validation;
   }
   ...
   public void selectWorld(int par1) {
      if (!this.selected) {
         ...
         if (this.selectedWorldPassedValidation() && this.mc.getSaveLoader().canLoadWorld(var2)) {
            while(!ThreadedFileIOBase.isFinished()) {
               try {
                  Thread.sleep(500L);
            ...
            this.mc.increment_loadWorldStat_asap = true;
         } else {
            this.selected = false;
         }
      }
   ...
   public static GuiYesNoMITE getDeleteWorldScreen(GuiScreen par0GuiScreen, String par1Str, int par2) {
      ...
      GuiYesNoMITE var7 = new GuiYesNoMITE(par0GuiScreen, var3, var4, var5, var6, par2);
      return var7;
   }
   ...
   protected void keyTyped(char par1, int par2) {
      if (par2 == 1 && this.selectedWorld >= 0) {
         this.selected = false;
         this.selectedWorld = -1;
         getSelectButton(this).enabled = false;
         getRenameButton(this).enabled = false;
         getDeleteButton(this).enabled = false;
         func_82312_f(this).enabled = false;
      } else if (par2 == 28) {
         if (this.selectedWorld < 0) {
            this.actionPerformed(this.button_create);
         } else {
            this.actionPerformed(this.buttonSelect);
         }
      } else if (par2 == 211 && this.buttonDelete.isClickable()) {
