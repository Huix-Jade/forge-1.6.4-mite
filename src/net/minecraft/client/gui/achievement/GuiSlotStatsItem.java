   ...
   public GuiSlotStatsItem(GuiStats par1GuiStats) {
      ...
      while(var2.hasNext()) {
         ...
         if (GuiStats.this_world_only) {
            if (PlayerStatsHelper.getValueOnClient(var3) > 0L) {
               var4 = true;
            } else if (StatList.objectBreakStats[var5] != null && PlayerStatsHelper.getValueOnClient(StatList.objectBreakStats[var5]) > 0L) {
               var4 = true;
            } else if (StatList.objectCraftStats[var5] != null && PlayerStatsHelper.getValueOnClient(StatList.objectCraftStats[var5]) > 0L) {
               var4 = true;
            } else {
               var4 = false;
            }
